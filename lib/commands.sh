...
domain_warn_legacy_alias() {
  log_info "Command 'gas domain' adalah legacy alias. Disarankan pakai 'gas deploy'."
}

domain_legacy_ssl_to_deploy() {
  local value="$1"
  local normalized
  normalized="$(normalize_yes_no "$value" || true)"
  [[ -n "$normalized" ]] || die "Nilai --ssl harus yes atau no."
  if [[ "$normalized" == "yes" ]]; then
    printf 'certbot-nginx\n'
  else
    printf 'none\n'
  fi
}
...
run_domain() {
  ...
  case "$subcommand" in
    add)
      deploy_args+=(add --mode single-app)
      local domain_arg=""
      while [[ $# -gt 0 ]]; do
        case "$1" in
          --help|-h)
            ...
            return 0
            ;;
          --app|--port|--no-ui|--yes|-y)
            deploy_args+=("$1")
            if [[ "$1" == "--app" || "$1" == "--port" ]]; then
              [[ $# -ge 2 ]] || die "Flag $1 butuh nilai."
              deploy_args+=("$2")
              shift 2
              continue
            fi
          ...
          --ssl)
            ...
            deploy_args+=(--ssl "$(domain_legacy_ssl_to_deploy "$2")")
            shift 2
            ;;
          --ssl=*)
            deploy_args+=(--ssl "$(domain_legacy_ssl_to_deploy "${1#*=}")")
            shift
            ;;
          --app=*|--port=*)
            deploy_args+=("$1")
      ...
      [[ -n "$domain_arg" ]] && deploy_args+=(--domain "$domain_arg")
      domain_warn_legacy_alias
    ...
    remove|rm|delete)
      ...
      [[ -n "$domain_arg" ]] && deploy_args+=("$domain_arg")
      domain_warn_legacy_alias
      run_deploy "${deploy_args[@]}"
      ;;
    list|ls)
